...
class RepairWorkflowEngine:
    ...
    def _ensure_structure_toon(self, workspace: Path) -> Path:
        ...
        if not structure_path.exists():
            ...
            cdscan_path = Path(__file__).parent.parent / ".." / "cdscan" / "cdscan_cli.py"
            cmd = [
                sys.executable,
                str(cdscan_path),
    ...
    def _ensure_qa_report_toon(self, workspace: Path) -> Path:
        ...
        if not quality_path.exists():
            ...
            cdqa_path = Path(__file__).parent.parent / ".." / "cdqa" / "cdqa_cli.py"
            cmd = [
                sys.executable,
                str(cdqa_path),
                "--workspace", str(workspace.resolve()),
    ...
    def execute_pipeline(self, workspace: Path, test_case: str | None = None) -> Path:
        ...
        structure_data, quality_data = self.loader.load_full_analysis_data(structure_path, quality_path)
        
        deduplicated_failures = self.mapper.deduplicate_failures(failures)
        
        enriched_contexts, correlations, fixes = self.mapper.enrich_failures_with_correlation(
        ...
        fix_payload = self.builder.construct_fix_request(
            enriched_contexts,
            root_cause_analysis=correlations,
