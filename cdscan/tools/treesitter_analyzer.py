...
class TreeSitterAnalyzer:
    ...
    def _should_exclude_file(self, file_path: Path) -> bool:
        ...
        # Also exclude files with .min in name (minified files)
            ...
            if child.type == 'parameters':
                for param in child.children:
                    if param.type == 'identifier':
                        params.append(source[param.start_byte:param.end_byte].decode('utf-8'))
        return params
    ...
    def _extract_python_entry_point(self, node: 'Node', file_result: Dict, source: bytes):
        """
            ...
            if child.type == 'if_statement':
                ...
                for subchild in child.children:
                    if subchild.type == 'comparison_operator':
                        condition_text = source[subchild.start_byte:subchild.end_byte].decode('utf-8')
    ...
    def _extract_python_calls(self, node: 'Node', file_result: Dict, source: bytes):
        ...
        def traverse(n: 'Node', in_function: str = None):
            ...
            if n.type == 'call':
                ...
                for child in n.children:
                    if child.type == 'identifier':
                        called = source[child.start_byte:child.end_byte].decode('utf-8')
                        if in_function and called != in_function:  # Avoid self-recursion noise
                            calls_map.setdefault(in_function, set()).add(called)
                        break
                    elif child.type == 'attribute':
                        # method calls like obj.method()
                        attr_text = source[child.start_byte:child.end_byte].decode('utf-8')
                        if in_function:
                    ...
                    if subchild.type == 'identifier':
                        ...
                        for qchild in subchild.children:
                            if qchild.type == 'identifier':
                                return source[qchild.start_byte:qchild.end_byte].decode('utf-8')
        return 'unknown'
                            ...
                            if param.type == 'parameter_declaration':
                                ...
                                for pchild in param.children:
                                    if pchild.type == 'identifier':
                                        params.append(source[pchild.start_byte:pchild.end_byte].decode('utf-8'))
        return params
    ...
    def _build_import_graph(self) -> Dict[str, Any]:
        ...
        for file_info in self.results.get('files', []):
            ...
            for import_info in file_info.get('imports', []):
                ...
                module = self._extract_module_name(statement, language)
    ...
    def _build_call_graph(self) -> List[Dict[str, str]]:
        ...
        common_builtins = {'print', 'len', 'str', 'int', 'float', 'list', 'dict',
    ...
    def _simplify_imports(self, imports: List[Dict]) -> Dict[str, List[str]]:
        ...
        stdlib_modules = {
            'os', 'sys', 're', 'json', 'typing', 'pathlib', 'logging',
            'datetime', 'collections', 'itertools', 'functools', 'operator',
            'subprocess', 'argparse', 'unittest', 'io', 'tempfile', 'shutil',
            'copy', 'math', 'random', 'time', 'threading', 'multiprocessing',
            'socket', 'http', 'urllib', 'email', 'html', 'xml', 'sqlite3',
            'csv', 'pickle', 'hashlib', 'base64', 'ast', 'inspect', 'traceback',
            'contextlib', 'abc', 'dataclasses', 'enum', 'textwrap', 'string',
            'struct', 'array', 'queue', 'heapq', 'bisect', 'weakref',
            'types', 'warnings', 'dis', 'gc', 'platform', 'signal',
        }
    ...
    def _extract_module_name(self, statement: str, language: str) -> Optional[str]:
        """
        ...
        if language == 'python':
            ...
            # #include <module> or #include "module"
