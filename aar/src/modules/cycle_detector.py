...
from modules.data_types import PatchToon, CycleDetectionState
...
class CycleDetector:
    ...
    def check_duplicate_patch(self, patch: PatchToon) -> bool:
        """
        Check if the same patch occurred consecutively
        ...
        if self.state.last_patch is not None:
            # Check if it's an exact duplicate (same location and same new_code)
            is_exact_duplicate = (
            ...
            if is_exact_duplicate:
                logger.warning(f"Exact duplicate patch detected at {patch.file_path}:{patch.line_range}")
                self.state.last_patch = patch
                self.state.last_patch_hash = current_hash
                return True
            
            # Check if it's a semantic refinement (same location, similar new_code)
            if (self.state.last_patch.file_path == patch.file_path
                and self.state.last_patch.line_range == patch.line_range):
                
                # Calculate similarity between new_code values
                similarity = self._calculate_code_similarity(
                    self.state.last_patch.new_code,
                    patch.new_code
                )
                
                # If very similar (>90%), it's likely a refinement, not a cycle
                if similarity > 0.9:
                    logger.info(f"Semantic refinement detected (similarity: {similarity:.2f})")
                    self.state.last_patch = patch
                    self.state.last_patch_hash = current_hash
                    return False
                
                # If similar but different enough, it's a new approach
                if similarity > 0.5:
                    logger.info(f"Different approach detected (similarity: {similarity:.2f})")
                    self.state.last_patch = patch
                    self.state.last_patch_hash = current_hash
                    return False
    ...
    def check_signature_cycle(self, signatures: Set[str]) -> bool:
        ...
        if len(self.state.signature_history) > self.window_size:
            self.state.signature_history = self.state.signature_history[-self.window_size :]
