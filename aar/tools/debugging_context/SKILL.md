---
name: debugging-context-generator
...
# Debugging Context Generator Skill
...
Assistant: [Uses Skill tool with skill: "debugging-context-generator"]
```
...
```bash
cd /home/vscode/.claude/skills/debugging_context
python -m debugging_context /path/to/workspace
```
...
python -m debugging_context /workspaces/my-project

# Focus on specific test file
python -m debugging_context /workspaces/my-project --test-case tests/test_math.py

# Debug single test function
python -m debugging_context /workspaces/my-project --test-case tests/test_math.py::test_addition
```

## What This Skill Does

### 1. Artifact Loading
- **Load static analysis**: Reads `structure.toon` (codebase structure from code-analyzer)
- **Load quality data**: Reads `qa_report.toon` (quality issues from code-QA)
- **Auto-generate missing**: Automatically runs `code_analyzer` and `code_QA` if TOON files don't exist
...
**2. code_analyzer** (optional, auto-invoked)
- Pre-installed as sibling skill
- Auto-runs if `structure.toon` missing

**3. code_QA** (optional, auto-invoked)
- Pre-installed as sibling skill
  ...
  - repo: local
    hooks:
      - id: debug-context
        ...
        entry: python -m debugging_context .
        language: system
...
python -m debugging_context . --test-case tests/
```
...
# - code_analyzer (for structure.toon)
# - code_QA (for qa_report.toon)

# To manually generate:
cd /home/vscode/.claude/skills/code_analyzer
...
| Aspect | code-analyzer | code-QA | debugging-context-generator |
|--------|---------------|---------|----------------------------|
